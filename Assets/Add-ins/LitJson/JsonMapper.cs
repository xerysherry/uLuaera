...
namespace LitJson
{
    ...
    public class JsonMapper
    {
        ...
        private static object ReadVal(Type inst_type, JsonData jd)
        {
            // support float
            if (inst_type == typeof(float) && jd.IsDouble)
            {
                return (float)jd;
            }
            ...
            if (jd.IsInt)
                ...
                return ReadValue(inst_type, new JsonReader(jd.ToJson()));

            Type json_type = jdval.GetType();
            ...
            if (inst_type.IsEnum)
            {
                ...
                {
                    return Enum.ToObject(inst_type, jdval);
                }
                if (json_type == typeof (string))
                {
                    int temp = 0;
                    if (int.TryParse((string) jdval, out temp))
                    {
                        return Enum.ToObject(inst_type, temp);
                    }
                }
                return Enum.ToObject(inst_type, jdval);
            }
            ...
            if (json_type == typeof (string))
            {
                return Convert.ChangeType(jdval, inst_type);
            }
        ...
        public static bool SetObjectValue(object instance, string path, JsonData jsonData)
        {
            Type inst_type = instance.GetType();
            try
            {
                if (inst_type.IsValueType)
                    throw new Exception("can't set value type");
                ...
                if (string.IsNullOrEmpty(path))
                {
                    if (jsonData == null || !jsonData.IsObject)
                        throw new Exception("jsonData is not Object");
                    foreach (DictionaryEntry entry in ((IDictionary) jsonData))
                    {
                        SetObjectValue(instance, (string) entry.Key, (JsonData) entry.Value);
                    }
                }
                else
                {
                    ...
                    if (dotPos >= 0)
                    {
                        property = path.Substring(0, dotPos);
                            ...
                            if (string.IsNullOrEmpty(nextpath))
                            {
                                if (jsonData != null)
                                {
                                    ((FieldInfo) prop_data.Info).SetValue(
                                        instance, ReadVal(prop_data.Type, jsonData));
                                }
                                else
                                {
                                    //only class can be deleted
                                    if (((FieldInfo) prop_data.Info).FieldType.IsClass)
                                        ((FieldInfo) prop_data.Info).SetValue(instance, null);
                                }
                            }
                            else
                            {
                                object nextObj = ((FieldInfo) prop_data.Info).GetValue(instance);
                                SetObjectValue(nextObj, nextpath, jsonData);
                            }
                        }
                        else
                        {
                            ...
                            if (p_info.CanWrite)
                            {
                                if (string.IsNullOrEmpty(nextpath))
                                {
                                    if (jsonData != null)
                                    {
                                        p_info.SetValue(instance, ReadVal(prop_data.Type, jsonData), null);
                                    }
                                    else
                                    {
                                        //only class can be deleted
                                        if (p_info.PropertyType.IsClass)
                                            p_info.SetValue(instance, null, null);
                                    }
                                }
                                else
                                {
                                    SetObjectValue(p_info.GetValue(instance, null), nextpath, jsonData);
                                }
                            }
                        }
                    }
                    else
                    {
                        ...
                        {
                            throw new Exception(
                                string.Format("The type {0} doesn't have the property '{1}'",
                                    inst_type, property));
                        }
                        Object property_value = property;

                        Type[] argTypes = inst_type.GetGenericArguments();

                        property_value = Convert.ChangeType(property, argTypes[0]);

                        var dic_instance = (IDictionary) instance;
                        if (string.IsNullOrEmpty(nextpath))
                        {
                            if (jsonData != null)
                            {
                                if (dic_instance.Contains(property_value))
                                {
                                    dic_instance.Remove(property_value);
                                }

                                dic_instance.Add(property_value,
                                    ReadVal(argTypes[1], jsonData));
                            }
                            else
                            {
                                dic_instance.Remove(property_value);
                        ...
                        {
                            if (argTypes[1].IsValueType)
                                throw new Exception("can not change value type partly");

                            if (dic_instance.Contains(property_value))
                            {
                                SetObjectValue(dic_instance[property_value], nextpath, jsonData);
                            }
                            else
                            {
                                throw new Exception(string.Format("The dictionary {0} doesn't have the " +
                                                                  "key '{1}'", inst_type, property));
                            }
                        }
                    }
                }
            ...
            {
                throw new Exception(string.Format("SetObjectValue({0}, {1}, {2})", instance, path, jsonData.ToJson()), e);
            }
            return true;
        }
        ...
        public static JsonData ToObject(string json)
        {
            if (string.IsNullOrEmpty(json))
                return new JsonData(string.Empty);
        ...
        public static object ToObject(string json, Type t)
        {
            ...
            return ReadValue(t, json_reader);
        }
        ...
        public static object ReadValue(Type inst_type, JsonReader reader)
        {
                ...
                reader.Token == JsonToken.Boolean)
            {
                ...
                // support float
                if (inst_type == typeof (float) && reader.Token == JsonToken.Double)
                {
            ...
            else if (reader.Token == JsonToken.ObjectStart)
            {
                ...
                while (true)
                {
                    ...
                    else
                    {
                        ...
                        if (inst_type.GetInterface("IDictionary") != null)
                        {
                            Type[] argTypes = inst_type.GetGenericArguments();

                            property_value = Convert.ChangeType(property, argTypes[0]);
                            tempVal = ReadValue(argTypes[1], reader);
                        }
                        else
                        {
                            tempVal = ReadValue(t_data.ElementType, reader);
                        }

                        ((IDictionary) instance).Add(property_value, tempVal);
                    }
                }
            }
            ...
            int depth)
        {
            ...
            if (obj is IDictionary)
            {
                ...
                foreach (DictionaryEntry entry in (IDictionary) obj)
                {
					...
					// Dictionary<int, xx> 型无法使用(string)来转型
